...
download = unzip_data.TheiaDownload(
    folder_theia, tile_name="31TCJ", start_date="2017-01-01", end_date="2017-12-31"
)
...
rasterize_labels = stack_data.RasterLabels(
...
concatenate_images = stack_data.StackFoldersSentinel2(
    extent_vector=mask_data,
    bands=band_names,
...
gap_filling.GapFill(otb_path, path_output, bands=["B2", "B3", "B4", "B8"], res=10)

gap_filling.GapFill(
    otb_path, path_output, bands=["B5", "B6", "B7", "B8A", "B11", "B12"], res=20
)

# Put 20 meters images into 10 meters

workflow_si = superimpose.GFSuperImpose(
    otb_path, path_output, bands_20=["B5", "B6", "B7", "B8A", "B11", "B12"]
)
...
output_files = training_set.TrainingSet(
    path_images=path_output,
    band_names=features,
    dates=dates,
    saving_path=path_ts,
    reference_file=reference_file,
