...
class VegetationIndices:
    def __init__(self, saving_path="Sentinel2/GEOTIFFS"):
        self.saving_path = saving_path

    @staticmethod
    def _get_meta_infos(saving_path, times):
    ...
    def _write_tiff(self, array, variable_name, dimT, meta):
        path_tempo_file = os.path.join(
            self.saving_path, "TempoGFstack_" + variable_name + "_crop_2019.tif"
        )
        ...
        topts = gdal.TranslateOptions(
            ...
            bandList=list(range(1, src_ds.RasterCount)),
        )  # gdal.GDT_Byte
    ...
    def compute_VIs(self, ECNorm=False):
        B3 = gdal.Open(
            os.path.join(self.saving_path, "GFstack_B3_crop_2019.tif")
        ).ReadAsArray()
        B4 = gdal.Open(
            os.path.join(self.saving_path, "GFstack_B4_crop_2019.tif")
        ).ReadAsArray()
        B8 = gdal.Open(
            os.path.join(self.saving_path, "GFstack_B8_crop_2019.tif")
        ).ReadAsArray()

        times = B3.shape[0]
        meta = self._get_meta_infos(self.saving_path, times)
        ...
        if ECNorm:
            ...
            array_bands_EC += -1
            self._write_tiff(array_bands_EC, "ECNorm", times, meta)

        # Vegetation indices
        NDVI = (B8 - B4) / (B8 + B4)
        del B4
        self._write_tiff(NDVI, "NDVI", times, meta)
        del NDVI

        NDWI = (B3 - B8) / (B3 + B8)
        del B3
        del B8
        self._write_tiff(NDWI, "NDWI", times, meta)
