...
if len(sys.argv) == 1:
    ...
    print("        ou : ", prog, " -h")
    print(
        "example 1 : python %s -t 'T31TFJ' -a config.cfg -d 2018-07-01 -f 2018-07-31"
        % sys.argv[0]
    )
    print(
        "example 2 : python %s -l 'Toulouse' -a config.cfg -d 2018-07-01 -f 2018-07-31 --level LEVEL3A"
        % sys.argv[0]
    )
    print(
        "example 3 : python %s --lon 1 --lat 44 -a config.cfg -d 2015-12-01 -f 2015-12-31"
        % sys.argv[0]
    )
    print(
    ...
    print(
        "example 6 : python %s -l 'France' -c VENUS -a config.cfg -d 2018-01-01"
        % sys.argv[0]
    )
    print(
        "example 7 : python %s -s 'KHUMBU' -c VENUS -a config.cfg -d 2018-01-01"
        % sys.argv[0]
    )
    print(
        "example 8 : python %s -l 'France' -c LANDSAT -a config.cfg -d 2018-01-01"
        % sys.argv[0]
    )
    sys.exit(-1)
else:
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="Tile number (ex: T31TCJ), Sentinel2 only",
        default=None,
    )
    parser.add_option(
        "--lat",
    ...
    parser.add_option(
        ...
        action="store",
        dest="level",
        choices=["LEVEL1C", "LEVEL2A", "LEVEL3A"],
        help="product level for reflectance products",
        default="LEVEL2A",
    )
...
if options.tile == None:
    if options.location == None and options.site == None:
        if options.lat == None or options.lon == None:
            if (
                options.latmin == None
                or options.lonmin == None
                or options.latmax == None
                or options.lonmax == None
            ):
            ...
            if (
                options.latmin == None
                and options.lonmin == None
                and options.latmax == None
                and options.lonmax == None
            ):
                geom = "point"
            else:
...
if geom == "point":
    ...
    dict_query = {
        "box": "{lonmin},{latmin},{lonmax},{latmax}".format(
            latmin=options.latmin,
            latmax=options.latmax,
            lonmin=options.lonmin,
            lonmax=options.lonmax,
...
get_token = (
    'curl -k -s -X POST %s --data-urlencode "ident=%s" --data-urlencode "pass=%s" %s/services/authenticate/>token.json'
...
try:
    for i in range(len(data["features"])):
        ...
        get_product = (
            'curl %s -o "%s" -k -H "Authorization: Bearer %s" %s/%s/collections/%s/%s/download/?issuerId=theia'
            % (
                curl_proxy,
                tmpfile,
                token,
