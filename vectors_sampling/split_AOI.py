...
def get_gridded_boundary(shapefile, cell_size=1000):
    ...
    def get_shape_area(bbox, distance=cell_size):
        ...
        c1 = int(
            (bbox["maxx"] - bbox["minx"]) / distance
        )  # + int((bbox['maxx'] - bbox['minx'])%distance)
        # Number of horizontal patch xmin - xmax
        c2 = int(
            (bbox["maxy"] - bbox["miny"]) / distance
        )  # + int((bbox['maxy'] - bbox['miny'])%distance)
        return (c1, c2)
    ...
    gdf = gpd.GeoDataFrame(
        {"index_x": idxs_x, "index_y": idxs_y}, crs=shapefile.crs, geometry=geometry
    )
