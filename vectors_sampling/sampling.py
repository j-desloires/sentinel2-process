...
def random_sample_vector(vector_file, alpha, column_class = 'Class'):
    ...
    vector_file['Count'] = 1
    pivot = pd.pivot_table(vector_file[[column_class,'index','Count']],
                           index = 'index',
    ...
    for index_grid in pivot['index']:
        for Class in list(vector_file[column_class].unique()):
            ...
            if vector_file.loc[(vector_file['index'] == index_grid) & (vector_file[column_class] == Class)].shape[0] > theoretical_number:
                sample = vector_file.loc[(vector_file['index'] == index_grid) & (vector_file[column_class] == Class)].sample(int(theoretical_number+1))
            else:
                sample = vector_file.loc[(vector_file['index'] == index_grid) & (vector_file[column_class] == Class)]
